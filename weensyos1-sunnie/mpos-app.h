 ...
 *   Each system call is defined by assembly code th3333at implements a protected
 ...
 * sys_kill
 *	
 	Exercize 7: From spec:
 	Introduce a sys_kill(pid) system call by which one process can 
 	make another process exit. Use this system call to alter 
 	mpos-app2.c's run_child() function so that the even-numbered 
 	processes kill off all odd-numbered processes (except for 
 	thread 1). Running the application should print out 
 	"Process N lives" messages only for even-numbered values of N.
 *   
 *****************************************************************************/
...
sys_kill(pid_t pid)
{
	// retval returns 0 upon success. -1 upon error (Error if pid is not a 
	// valid pid or pid is 1) <== We don't want to kill the main process.
	pid_t retval;
	asm volatile("int %1\n"
		     : "=a"(retval)
		     : "i" (INT_SYS_KILL),
...
{
	//return value: -1 		if fail to create thread, 
			//		0 		if return from thread,
			//	thread pid 	if return from parent.
	pid_t retval;
	asm volatile("int %1\n"
			 : "=a"(retval)
			 : "i" (INT_SYS_NEWTHREAD),
