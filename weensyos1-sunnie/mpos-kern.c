...
static pid_t create_thread(process_t *parent, uint32_t starting_func);
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		// unblocked processes that wait on this process 
		// and set those processes' eax to the current exit status.
		pid_t pid;
		for(pid = 1; pid != NPROCS; pid++){
			if(proc_array[pid].p_state == P_BLOCKED && proc_array[pid].p_waitOn == current->p_pid){
	...
	case INT_SYS_KILL:{
		// load the pid of the process to be killed.
		pid_t pid = current->p_registers.reg_eax;
		// kill the target pid 
		if(pid < 1 || pid >= NPROCS || proc_array[pid].p_state == P_EMPTY)
			current->p_registers.reg_eax = -1;
		else{
			proc_array[pid].p_state = P_ZOMBIE;
			// Same as INT_SYS_KILL: unblocked processes that 
			// wait on the target process.
			pid_t pidB; //iterator
			for(pidB = 1; pidB != NPROCS; pidB++){
				if(proc_array[pidB].p_state == P_BLOCKED && proc_array[pidB].p_waitOn == pid)
					proc_array[pidB].p_state = P_RUNNABLE;
			}
			// set the target process exit status to 1. I am assuming
			// the user is not able to assign exit status to the target process
		 	proc_array[pid].p_exit_status = 1;
			// set the return value to 0 upon success.
	...
	case INT_SYS_NEWTHREAD:{
		//Exercise 6: Create a new "thread"
		//set return value to the pid of new generated thread;
																//starting function address.
...
do_fork(process_t *parent)
{
	...
	for(j = 1; j != NPROCS; j++){
		if(proc_array[j].p_state == P_EMPTY){
			child = &proc_array[j];
...
copy_stack(process_t *dest, process_t *src)
{
	...
	uint32_t offset = src_stack_top - src_stack_bottom;
	dest_stack_bottom = dest_stack_top - offset;
	// YOUR CODE HERE: memcpy the stack and set dest->p_registers.reg_esp
	memcpy((void*)dest_stack_bottom, (void*) src_stack_bottom, offset);
 ...
 * create_thread
 *
 	Exercise 6: From spec:
 	Introduce a new system call that creates a new process in a 
 	thread-like way. The new process should start with an empty 
 	stack, not a copy of the current stack. Rather than starting 
 	at the same instruction as the parent, the new thread should 
 	start by executing the start_function function: that is, that 
 	function's address becomes the new thread's instruction pointer.
 *****************************************************************************/

static pid_t
create_thread(process_t *parent, uint32_t starting_func )
{
	pid_t j;	
	...
	for(j = 1; j != NPROCS; j++){
		if(proc_array[j].p_state == P_EMPTY){
			...
			thread->p_registers = parent->p_registers;
			//set eip to the address of the function.
			thread->p_registers.reg_eip = starting_func;
			//same as do_fork, return value of thread should be 0.
			thread->p_registers.reg_eax = 0;
